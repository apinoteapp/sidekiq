namespace :load do
  task :defaults do
    ...
    set :sidekiq_options,       ->{ "-e #{fetch(:rails_env, 'production')} -C #{current_path}/config/sidekiq.yml -L #{current_path}/log/sidekiq.log > /dev/null 2>&1 &" }
      ...
      within current_path do
        for_each_process do |pid_file, idx|
          if fetch(:sidekiq_cmd)
            execute :nohup, fetch(:sidekiq_cmd), "-i #{idx} -P #{pid_file} #{fetch(:sidekiq_options)}"
          else
            execute :nohup, :bundle, :exec, :sidekiq, "-i #{idx} -P #{pid_file} #{fetch(:sidekiq_options)}"
          end
        end
      end
    end
